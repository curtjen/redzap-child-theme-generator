...
function rzct_options_page() {
	...
	<div class="wrap">
		...
		<?php
		$opts = array(
			'name' => esc_html__('Theme Name', 'rzct'),
			'template' => esc_html__('Template', 'rzct'),
		...
		foreach ($opts as $slug=>$title) {
			$value = '';
			if (!empty($results['rzct_'.$slug])) $value = esc_attr($results['rzct_'.$slug]);
			echo "<tr valign='top'><th scope='row'>{$title}</th><td><input class='regular-text' type='text' name='rzct_{$slug}' value='{$value}'></td></tr>\n";
		}
		?>
		</table>
